...
def answer_with_search(question,
    ...
    if verbosity > 1:
        pretty_print("OKGREEN", f"Generate Model: {model}")
        pretty_print("OKGREEN", f"Embedding Model: {embedding_model}")
    ...
    if not paragraphs:
        ...
        return ("", "", "")
    sample_answer = get_sample_answer(question, co, model=model)
    if verbosity > 1:
